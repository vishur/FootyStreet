...
function writeBookmarkLink(url, title, text, img) {
    var insert = '';
    if (img) insert = writeBookmarkLinkObject(url, title, '<img src="' + img + '" alt="' + escape(text) + '" title="' + escape(text) + '" />') + '&nbsp';
    insert += writeBookmarkLinkObject(url, title, text);
    if (window.sidebar || window.opera && window.print || (window.external && ('AddFavorite' in window.external))) document.write(insert);
}

function writeBookmarkLinkObject(url, title, insert) {
    if (window.sidebar || window.external) return ('<a href="javascript:addBookmark(\'' + escape(url) + '\', \'' + escape(title) + '\')">' + insert + '</a>');
    else if (window.opera && window.print) return ('<a rel="sidebar" href="' + escape(url) + '" title="' + escape(title) + '">' + insert + '</a>');
    return ('');
}
...
function ceilf(value, precision) {
    ...
    if (tmp2.indexOf('.') === false) return (value);
    if (tmp2.charAt(tmp2.length - 1) == 0) return value;
...
function updateTextWithEffect(jQueryElement, text, velocity, effect1, effect2, newClass) {
    if (jQueryElement.text() !== text) if (effect1 === 'fade') jQueryElement.fadeOut(velocity, function () {
...
function updateDisplay() {
    ...
    if (!selectedCombination['unavailable'] && quantityAvailable > 0 && productAvailableForOrder == 1) {
        ...
        if (!selectedCombination['unavailable']) $('#availability_value').text(doesntExistNoMore + (globalQuantity > 0 ? ' ' + doesntExistNoMoreBut : '')).addClass('warning_inline');
        else {
    ...
    if (!selectedCombination['unavailable'] && productShowPrice == 1) {
        ...
        if (selectedCombination.specific_price && selectedCombination.specific_price['id_product_attribute']) {
            if (selectedCombination.specific_price['price'] && selectedCombination.specific_price['price'] >= 0) var taxExclPrice = (specific_currency ? selectedCombination.specific_price['price'] : selectedCombination.specific_price['price'] * currencyRate);
            else var taxExclPrice = productBasePriceTaxExcluded * currencyRate + (selectedCombination['price'] * currencyRate);
        } else if (product_specific_price.price && product_specific_price.price >= 0) var taxExclPrice = (specific_currency ? product_specific_price.price : product_specific_price.price * currencyRate) + (selectedCombination['price'] * currencyRate);
        if (!displayPrice && !noTaxForThisProduct) productPriceDisplay = taxExclPrice * tax;
        ...
        if ((product_specific_price['reduction_type'] == 'percentage' && selectedCombination['specific_price'].reduction_type == 'percentage') || selectedCombination['specific_price'].reduction_type == 'percentage') $('#reduction_percent').show();
...
function displayImage(domAAroundImgThumb, no_animation) {
    ...
    if (domAAroundImgThumb.attr('href')) {
        ...
        if ($('#bigpic').attr('src') != newSrc) {
            ...
            if (typeof (jqZoomEnabled) != 'undefined' && jqZoomEnabled) $('#bigpic').attr('rel', domAAroundImgThumb.attr('href')), $('#zoom1').attr('href', domAAroundImgThumb.attr('href')), $('#mousetrap_img').attr('rel', domAAroundImgThumb.attr('href')).attr('src', domAAroundImgThumb.attr('href'))
        }
...
$(document).ready(function () {
    ...
    if (typeof (jqZoomEnabled) != 'undefined' && jqZoomEnabled) {
        ...
        $('#zoom1').attr('href', $('#bigpic').attr('src').replace('large', 'thickbox')), $('#mousetrap_img').attr('rel', $('#bigpic').attr('src').replace('large', 'thickbox')).attr('src', $('#bigpic').attr('src').replace('large', 'thickbox'))
    }
...
function getProductAttribute() {
    ...
    if (attributesCombinations[i]['id_attribute'] === tab_attributes[a]) request += '/' + attributesCombinations[i]['group'] + '-' + attributesCombinations[i]['attribute'];
...
(function ($) {
    $.fn.jqueryzoom = function (options) {
        ...
        $(this).hover(function () {
            ...
            if (settings.position == "right") leftpos = imageRelativeLeft + imageWidth + settings.offset;
            else leftpos = imageRelativeLeft - settings.xzoom - settings.offset;
            $("div.zoomdiv").css({
...
function updateStateByIdCountry() {
    ...
    var query = $.ajax({
        ...
        success: function (json) {
            ...
            if (SE_RefreshMethod == 0) updateCarriersList();
    ...
    $('#availableCarriers').slideUp('normal', function () {
        ...
        var query = $.ajax({
            ...
            success: function (json) {
                if (json.length) {
                    for (carrier in json) {
                        var html = '<tr class="' + (carrier % 2 ? 'alternate_' : '') + 'item">' + '<td class="carrier_action radio">' + '<input type="radio" name="id_carrier" value="' + json[carrier].id_carrier + '" id="id_carrier' + json[carrier].id_carrier + '" ' + (id_carrier == json[carrier].id_carrier ? 'checked="checked"' : '') + '/>' + '</td>' + '<td class="carrier_name">' + '<label for="id_carrier' + json[carrier].id_carrier + '">' + (json[carrier].img ? '<img src="' + json[carrier].img + '" alt="' + json[carrier].name + '" />' : json[carrier].name) + '</label>' + '</td>' + '<td class="carrier_infos">' + ((json[carrier].delay != null) ? json[carrier].delay : '') + '</td>' + '<td class="carrier_price">';
...
var ajaxCart = {
    ...
    updateCartInformation: function (jsonData, addedFromProductPage) {
        ajaxCart.updateCart(jsonData);
        if (addedFromProductPage) $('#add_to_cart input').removeAttr('disabled').addClass('exclusive').removeClass('exclusive_disabled');
        else $('.ajax_add_to_cart_button').removeAttr('disabled');
    },
    add: function (idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist) {
        if (addedFromProductPage && !checkCustomizations()) {
            $('body').append('<div id="myModal" class="modal hide fade notification notification_warning" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>' + fieldRequired + '</div>'), $('#myModal').modal('show');
    ...
    remove: function (idProduct, idCombination, customizationId, idAddressDelivery) {
        $.ajax({
            ...
            error: function () {
                $('body').append('<div id="myModal" class="modal hide fade notification notification_warning" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>ERROR: unable to delete the product</div>'), $('#myModal').modal('show');
                $('#myModal').click(function () {
                    $('#myModal').modal('hide');
                    $(this).remove();
                });
    ...
    hideOldProductCustomizations: function (product, domIdProduct) {
        ...
        if (!product.hasCustomizedDatas && !removeLinks.length) $('#' + domIdProduct + ' span.remove_link').html('<a class="ajax_cart_block_remove_link" rel="nofollow" href="' + baseUri + '?controller=cart&amp;delete=1&amp;id_product=' + product['id'] + '&amp;ipa=' + product['idCombination'] + '&amp;token=' + static_token + '"><i class="icon-trash"></i></a>');
        if (product.is_gift) $('#' + domIdProduct + ' span.remove_link').html('');
    },
        ...
        $(jsonData.products).each(function () {
            if (this.id != undefined) {
                if ($('#cart_block dl.products').length == 0) {
                    ...
                    $('#header .cart-prices, #header #cart-buttons').show();
                }
                ...
                if ($('#cart_block_product_' + domIdProduct).length == 0) {
                    ...
                    if (parseFloat(this.price_float) > 0) content += '<span class="remove_link"><a rel="nofollow" class="ajax_cart_block_remove_link" href="' + baseUri + '?controller=cart&amp;delete=1&amp;id_product=' + productId + '&amp;token=' + static_token + (this.hasAttributes ? '&amp;ipa=' + parseInt(this.idCombination) : '') + '"><i class="icon-trash"></i></a></span>';
                    ...
                    if ($.trim($('#cart_block_product_' + domIdProduct + ' .quantity').html()) != jsonProduct.quantity || $.trim($('#cart_block_product_' + domIdProduct + ' .price').html()) != jsonProduct.priceByLine) {
                        if (!this.is_gift) $('#cart_block_product_' + domIdProduct + ' .price').text(jsonProduct.priceByLine);
                        else $('#cart_block_product_' + domIdProduct + ' .price').html(freeProductTranslation);
                        ajaxCart.updateProductQuantity(jsonProduct, jsonProduct.quantity);
                        if (jsonProduct.hasCustomizedDatas) {
                            customizationFormatedDatas = ajaxCart.displayNewCustomizedDatas(jsonProduct);
                            if (!$('#customization_' + domIdProductAttribute).length) {
                                if (jsonProduct.hasAttributes) $('#cart_block_combination_of_' + domIdProduct).append(customizationFormatedDatas);
    ...
    updateCart: function (jsonData) {
        if (jsonData.hasError) {
            ...
            $('body').append('<div id="myModal" class="modal hide fade notification notification_warning" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>' + errors + '</div>'), $('#myModal').modal('show');
            $('#myModal').click(function () {
                $('#myModal').modal('hide');
                $(this).remove();
            });
